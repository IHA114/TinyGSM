...
bool LBS_Connect()
{
  ...
  // Open a GPRS context
   ...
   //waitResponse(85000L);
    // Query the GPRS context
    sendAT(GF("+SAPBR=2,1"));
   if (waitResponse(85000L,GF(GSM_NL "+SAPBR:")) != 1)
	 {
	    TINY_GSM_DEBUG.println("Query the GPRS context failure!");
      return false;
	 }
   waitResponse(1000L);
  	// Get customer ID	
    sendAT(GF("+CLBSCFG=0,1"));
    if (waitResponse(GF(GSM_NL "+CLBSCFG: 0,1,")) != 1)
    {
      TINY_GSM_DEBUG.println("Get customer ID failure!");
      return false;
    }

    //Get Times have used positioning command
    sendAT(GF("+CLBSCFG=0,2")); 
    if (waitResponse(GF(GSM_NL "+CLBSCFG: 0,2,")) != 1)
    {
      TINY_GSM_DEBUG.println("Get Times have used positioning command!");
      return false;
    }
...
String Get_LOCData()
{
   //LBS_Connect(); 
    sendAT(GF("+CLBS=1,1"));
    if (waitResponse(85000L,GF(GSM_NL "+CLBS:")) != 1) {
      return "";
    }
    //waitResponse(85000L);
    String res = stream.readStringUntil('\n');
  ...
  String getGsmLocation() {
    sendAT(GF("+CIPGSMLOC=4,1"));
    if (waitResponse(30000L, GF(GSM_NL "+CIPGSMLOC:")) != 1) {
      return "";
    }
    String res = stream.readStringUntil('\n');
    waitResponse();
  ...
  /*
    Boot or Close GPS POWER
  */
  bool GPS_PWR_CONTROL(bool STATUS)
  {
	  if(STATUS==ON)
	  {
		 sendAT(GF("+CGNSPWR=1")); 
		 ...
		 {
		  TINY_GSM_DEBUG.println("GPS BOOT Fail!");
		  return false;
		 }
	  }
	  else
   ...
   /*
    config GPS  Parameter
  */ 
  bool GPS_Set_parameter(void)
  {
	//set GPS uart baud 
  //Set GPS Data Auto Output Disable
  sendAT(GF("+CGNSTST=0")); 
  waitResponse();
	 sendAT(GF("+CGNSIPR=115200")); 
   waitResponse();
   /*
   delay(100);
	 if (waitResponse(2000L) != 1)
	 {
		TINY_GSM_DEBUG.println("GPS Uart BaudRate Set failure!"); 
		return false;
	 }
	 */
	 sendAT(GF("+CGNSSEQ=\"RMC\"")); 
	 waitResponse();
   /*
   delay(100);
	 if (waitResponse(1000L) != 1)
	 {
		TINY_GSM_DEBUG.println("GPS Set Param failure!"); 
		return false;
	 }
   */
	return true; 
  ...
  delay(100);
    if (waitResponse(GF(GSM_NL "+CGNSINF:")) != 1) {
	  ...
	  TINY_GSM_DEBUG.println("Get GPS Data  failure!");
      return "";
    }	
    String res = stream.readStringUntil('\n');
    waitResponse();
	//TINY_GSM_DEBUG.println(res);
    res.trim();
    return res;	
  }
